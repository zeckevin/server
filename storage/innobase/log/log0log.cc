...
void log_t::file::set_file_path(std::string path)
{
...
void log_t::file::open_file()
{
  ...
  bool success;
  fd= os_file_create(innodb_log_file_key, file_path.c_str(),
                     OS_FILE_OPEN | OS_FILE_ON_ERROR_NO_EXIT, OS_FILE_NORMAL,
                     OS_LOG_FILE, srv_read_only_mode, &success);
  if (!success)
    ib::fatal() << "os_file_create(" << file_path << ") failed";
}
...
void log_t::file::read(size_t offset, span<byte> buf)
{
  ...
  if (const dberr_t err=
          os_file_read(IORequestRead, fd, buf.data(), offset, buf.size()))
  {
    ib::fatal() << "os_file_read(" << file_path << ") returned " << err;
  }
}
...
void log_t::file::write(size_t offset, span<byte> buf)
{
  ...
  if (const dberr_t err= os_file_write(IORequestWrite, file_path.c_str(), fd,
                                       buf.data(), offset, buf.size()))
  {
    ib::fatal() << "os_file_write(" << file_path << ") returned " << err;
  }
}
...
void log_t::file::fsync()
{
  ...
  if (!os_file_flush(fd))
    ib::fatal() << "os_file_flush(" << file_path << ") failed";
...
void log_t::file::fdatasync()
{
  ...
  log_sys.pending_flushes.fetch_add(1, std::memory_order_acquire);
  if (!os_file_flush_data(fd))
    ib::fatal() << "os_file_flush_data(" << file_path << ") failed";
  log_sys.pending_flushes.fetch_sub(1, std::memory_order_release);
  log_sys.flushes.fetch_add(1, std::memory_order_release);
}
...
void log_t::file::close_file()
{
  ...
  if (!os_file_close(fd))
    ib::fatal() << "os_file_close(" << file_path << ") failed";
  fd= OS_FILE_CLOSED;
}
